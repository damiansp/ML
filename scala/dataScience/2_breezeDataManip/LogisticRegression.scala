...
class LogisticRegression(val training: DenseMatrix[Double], val target: DenseMatrix[Double]) {
  def costFunctionAndGradient(coefficients: DenseMatrix[Double]): (Double, DenseVector[Double]) = {
  ...
  private def calculateOptimalCoefficients: DenseVector[Double] = {
    val f = new DiffFunction[DenseVector[Double]] {
      def calculate(parameters: DenseVector[Double]) = costFunctionAndGradient(parameters)
    }
