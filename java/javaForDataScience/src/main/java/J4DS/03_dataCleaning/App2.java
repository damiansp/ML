...
public class App2 {
  public static void main(String[] args) {
    String dirtyText = (
      ...
      + " part of the world.");
    /*
    Spltter split = Splitter.on(',').omitEmptyStrings().trimResults();
    Iterable<String> words = split.split(dirtyText);

    for (String token: words) out.println(token);
  ...
  public static String findReplaceGuava(String text) {
    ...
    String spaced = CharMatcher.WHITESPACE.trimAneCollapseFrom(text, ' ');

    out.println("With double spaces removed: " + spaced);
    String noDigits = CharMatcher.JAVA_DIGIT.replace(text, "*");
