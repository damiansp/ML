...
def wait_for_agent_alias_status(
        agentId: str, agentAliasId: str, targetStatus: str):
            ...
            if 'trace' in trace:
                ...
                if 'orchestrationTrace' in trace_details:
                    ...
                    if 'invocationInput' in orch_trace:
                        ...
                        print("\nInvocation Input:")
                        print(
                            f"  Type: {inv_input.get('invocationType', 'N/A')}"
                        )
                        if 'actionGroupInvocationInput' in inv_input:
                            agi = inv_input['actionGroupInvocationInput']
                            print(
                                f"  Action Group: "
                                f"{agi.get('actionGroupName', 'N/A')}")
                            print(f"  Function: {agi.get('function', 'N/A')}")
                            print(
                                f"  Parameters: "
                                f"{agi.get('parameters', 'N/A')}")
                    ...
                    if 'observation' in orch_trace:
                        ...
                        if 'actionGroupInvocationOutput' in obs:
                            print(
                                f"  Action Group Output: "
                                f"{obs['actionGroupInvocationOutput'].get('text', 'N/A')}")
                        ...
                        if 'codeInterpreterInvocationOutput' in obs:
                            ...
                            print(
                                f"    Execution Output: "
                                f"{cio.get('executionOutput', 'N/A')[:50]}...")
                            print(
                                f"    Execution Error: "
                                f"{cio.get('executionError', 'N/A')}")
                            print(
                                f"    Execution Timeout: "
                                f"{cio.get('executionTimeout', 'N/A')}")
...
def wait_for_action_group_status(
    ...
    Args:
        ...
        targetStatus (str): The desired status to wait for. Defaults to
          'ENABLED'.
    Returns:
