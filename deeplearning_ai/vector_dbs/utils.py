...
def search_HNSW(GraphArray,G_query):
    ...
    for l_i in range(max_layers):
        ...
        nearest_distance_layer = np.linalg.norm(
            G_layer.nodes[entry_node_index]['pos'] - G_query.nodes['Q']['pos'])
        ...
        while cond:
            ...
            if nearest_distance_current < nearest_distance_layer:
                ...
                nodes_path.append(
                    (nearest_node_current,
                     {"pos": G_layer.nodes[nearest_node_current]['pos']}))
            else:
                cond = False
